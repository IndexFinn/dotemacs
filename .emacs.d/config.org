...
As I am not seeing the output of message at startup, I simply
insert it into the `*scratch*' buffer's header."
    (with-current-buffer (get-buffer "*scratch*")
      (save-excursion
        (beginning-of-buffer)
        (insert
         (format
          ";; EMACS started in %.3fs with %d garbage collections.\n"
          (float-time
...
Prompt for file completion from the directories defined in
`index/exwm-launch-mpv-dirs'.  Will focus the `mpv' buffer and
the selected file will be played.  If called with universal ARG,
only focus the buffer; if current buffer is the mpv buffer,
...
This function is auto generated via `index/exwm--raise-or-spawn'."
  ...
  (defun index/exwm-init-functions ()
    ...
    (prog1 t
      ...
      (add-to-list 'exwm-input-global-keys (kbd "<XF86AudioLowerVolume>"))
      (add-to-list 'exwm-input-prefix-keys (kbd "<XF86AudioRaiseVolume>"))
