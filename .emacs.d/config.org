...
(leaf emacs
  ...
  (defun index/rebuild-emacs-init ()
    ...
    (let* ((b (expand-file-name "config" user-emacs-directory))
      ...
      ;; session.
      (unless (s-equals? (sha1 (file->string o)) (file->string s))
        (dolist (f (list c n))
        ...
        (when (or (file-readable-p c)
                  (file-readable-p n))
          (delete-file e))
        ;; Write current `config.org' sha1 to `config.org.sha'.
        (with-temp-file s
          (insert (sha1 (file->string o)))))))
...
** DEPRECATED Small Font
:PROPERTIES:
:CUSTOM_ID: h:e3856980-ba71-4ec2-96dc-3d667afc06b4
:END:

*As of 2021-02-20 this section is deprecated; didn't really need a
small font version, though it might be cool to have.*
