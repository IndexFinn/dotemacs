...
#+author: Finn Sauer (finnsauer.com)
...
+ File Size: *src_elisp{(-> "config.org"
...
+ SRC-Blocks: *src_elisp{(length
                          (s-match-strings-all
                           ...
                           (buffer-string)))}*.
+ Document Lines: *src_elisp{(-> "config.org"
                                 (expand-file-name user-emacs-directory)
                                 file->list
                                 length
                                 1-)}*.
...
paradigm: the  focus lies  upon the  text.  The actual  code is  just an
addition to the  text and should merely underline the  thought.  This is
made possible via [[https://orgmode.org/][Org Mode]]-- the best [[https://daringfireball.net/projects/markdown/][Markdown]] out there.
#+end_abstract
...
#+toc: headlines insert TOC here with infinite headline levels
...
** Auxiliary Tools
:PROPERTIES:
:CUSTOM_ID: h:7941a36f-0443-44ba-a9d8-1612310d03a6
:END:
...
(leaf emacs
  ...
  (defun index/rebuild-emacs-init ()
    "Compile the ORG configuration file to ELC/ELN."
    (let* ((b (expand-file-name "config" user-emacs-directory))
           ...
           (c (s-append ".elc" b))
           (n (s-append ".eln" b)))
      ;; Don't compile if `config.org' hasn't changed since last
      ;; session.
      (unless (s-equals?
        ...
        ;; Compile to `config.eln' when native-comp is available.
...
*As of 2021-02-20 this section is under REVIEW: make it more
descriptive.*
...
buffers.  When you have another buffer open inside a window, that
buffer's background will be dimmed down.  This even works when accessing
the minibuffer-- this option can, of course, be turned off.
...
*As of 2021-02-20 this section is deprecated.  I didn't really use it
that much.  Though it could be nice for presentation purposes.*
...
Doom Emacs gained a lot of popularity.  Even though I encourage people
to craft their own configuration, it is nice for the lazy one.  But with
a lot of people working on it and great work by Hlsinger, there has been
an influx of new packages.  One of them is called: /Doom Modeline/.

This adds a quite nice looking modeline-- you would say “modern”
modeline, but the word modern kinda lost its meaning as everything is
modern these days.

Even though I am not opposed of the default modeline, this one looks
...
** COMMENT DEPRECATED Highlight S-Expressions
:PROPERTIES:
:CUSTOM_ID: h:30c3f344-4dbd-4ef3-86d2-3014180250d7
:END:

It's nice to know in what current AST-- abstract syntax tree-- level you
currently in.

#+begin_src emacs-lisp
(leaf highlight-sexp
  :straight t
  :hook ((prog-mode-hook . highlight-sexp-mode)))
#+end_src

** COMMENT DEPRECATED Highlight Symbol at Point
:PROPERTIES:
:CUSTOM_ID: h:f74a662e-77c3-4b40-b920-5918fbc0f706
:END:

Highlight every symbol that is the same as the symbol at point.
...
After each save the =delete-trailing-whitespace= function will-- as the
name implies-- remove all trailing white spaces.

A trailing white space is a Tab or Space at the end of a line without
any other character preceding it, making it useless.  In a way they take
up storage on the file system which-- with this function-- we can easily
mitigate it.  I know it's only a few bytes but still: a few bytes that
need to be stored and send over the network.

Disclaimer: by that nature it will delete the trailing new lines =\n= at
the end of the document too.  It makes sense if you think about it, but
can be rather annoying when you want to add a few new lines at the end
for coding purposes.  My best advice for escaping that: don't save the
buffer when adding new lines at the end of the document.
...
** COMMENT DEPRECATED View Mode
:PROPERTIES:
:CUSTOM_ID: h:17561d81-8a49-4510-b2d0-a715055eb628
:END:

*As of 2021-05-13, this section is deprecated; useless with god-mode.*
...
| Keybinding | Function Name  | Description                                               |
|------------+----------------+-----------------------------------------------------------|
| C-x C-e    | eval-last-sexp | Evaluate sexp before point; print value in the echo area. |
| C-M-x      | eval-defun     | This function has :override advice: ‘edebug-eval-defun’.  |
#+tblfm: $2='(key-binding (kbd $1))::$3='(ignore-errors (car (s-split "\n" (documentation (key-binding (kbd $1))))))
...
| Keybinding | Function Name       | Description                                         |
|------------+---------------------+-----------------------------------------------------|
| C-x C-e    | eros-eval-last-sexp | Wrapper for ‘eval-last-sexp’ that overlays results. |
| C-M-x      | eros-eval-defun     | Wrapper for ‘eval-defun’ that overlays results.     |
#+tblfm: $2='(key-binding (kbd $1))::$3='(ignore-errors (car (s-split "\n" (documentation (key-binding (kbd $1))))))
...
*As of 2021-02-23 this section is deprecated; there is no need for it if
I can spawn URxvt, frankly the best terminal emulator out there.*
...
** Tramp
:PROPERTIES:
:CUSTOM_ID: h:78cbf3db-6ab5-4d61-9408-a0483da96479
:END:
...
Setting the =auth-sources= to a /.gpg/ file is mandatory for obvious
reasons.  If you are ever asked to store a password, you can safely say
yes.
...
*** Appear
:PROPERTIES:
:CUSTOM_ID: h:ebc978c3-42eb-4073-91e4-3ff0cda87be2
:END:

*As of 2021-04-03 this section is under review; add more wording to this
section.*

When hovering over emphasis markers such as =*= or =/= it will display them,
otherwise hide them.
...
(leaf counsel
  ...
  (setq counsel-rg-base-command
        '("rg" "-M" "240" "--with-filename" "--no-heading"
          "--line-number" "--color" "never" "--hidden" "--follow"
          "--glob" "!{.git}" "%s"))
...
You can read up more about it on [[https://lipsum.com/][here]].
...
** Make Screenshot
:PROPERTIES:
:CUSTOM_ID: h:3021258e-22dc-42d0-9fec-b164a32b25fb
:END:
...
*As of 2021-02-20 this section is deprecated; I haven't really gone
around and tried Gnus-- you can see that since my old email address is
still here.  I will again try it in the future.*
...
3. If current buffer is window, switch to last buffer. "
    (let* ((buf-name-fn (intern
      ...
      `(prog1 t
         ...
         (defun ,buf-name-fn ()
           (sleep-for 0.1)
           (setq ,fn
                 (buffer-name index/last-created-exwm-buffer))
           (remove-hook
  ...
  (index/exwm-raise-or-spawn "brave" "s-p")
...
:PROPERTIES:
:CUSTOM_ID: h:0fed1b2a-5933-4146-a2cc-482cc6c0d2f6
:END:
** Web Development
:PROPERTIES:
...
*As of 2021-02-27 this section is under REVIEW: I feel like this section
is out of place and should be integrated elsewhere.*
...
To be able to use =s-u= or =H-u= as a universal argument is great, since
if for example a keybinding is mapped to =H-i= and that function can be
called with a universal argument to change some aspects of it, it's hard
to press =C-u H-i=.  With =H-u H-i= you don't have to move your finger
away from the modifier key.
...
** God Mode
:PROPERTIES:
:CUSTOM_ID: h:7da76747-6bbd-4381-917c-777a8cd244d1
:END:
...
(leaf god-mode
  ...
  (define-key isearch-mode-map (kbd "M-SPC") #'god-mode-isearch-activate)
  (define-key god-mode-isearch-map (kbd "M-SPC") #'god-mode-isearch-disable)
