...
paradigm; the focus lies more on the text than the actual code, and they
...
link to the repository can be found [[file:~/dot/emacs/.git/][here]].
...
The original file can be found [[file:init.el][here]].  The source is shown below:
...
The original file can be found [[file:early-init.el][here]].  The source is shown below:
...
* DEPRECATED Common Stuff
:PROPERTIES:
:CUSTOM_ID: h:39e38a6a-07dc-4c05-9db9-574396b93730
:END:

*The following has been DEPRECATED, as I want to move away from these
macros-- even though they are super nice*
...
As the author of this package states: "Are you tired of using the
endless keystrokes of =C-y M-y M-y M-y ...= to get at that bit of text you
killed thirty-seven kills ago?" ([[https://github.com/browse-kill-ring/browse-kill-ring][Synopsis on README]]).  This is indeed
one of my biggest problems with the kill ring.
  ...
  (defun index/org-goto-header ()
    ...
    (let ((res (completing-read
                ...
                (let ((temp nil))
                  (mapc
                   (lambda (str)
                     (when (s-match (concat "^" outline-regexp) str)
...
(leaf magit
  ...
  :commands (magit-status magit-edit-line-commit)
  :config
...
X window."
    (dolist (key (with-temp-buffer
                   ...
                   (let ((tmp nil))
                     (mapc
                      (lambda (str)
                        (when (s-match (concat "^" modifier "-") str)
    ...
    (index/call-process
     (concat
      (completing-read
       ...
       (flatten-tree
        (mapcar (lambda (str)
                  (directory-files-recursively str "."))
                (s-split ":" (getenv "PATH")))))
      (when arg
        (concat
         " "
  ...
  (defun index/ox-html ()
    ...
    (with-current-buffer (find-file-noselect
      ...
      (org-export-to-file 'html (expand-file-name
                                 "~/web/layouts/dotemacs/list.html")
        nil nil nil t nil)
      (with-current-buffer (find-file-noselect
                            (expand-file-name
                             "~/web/layouts/dotemacs/list.html"))
        (beginning-of-buffer)
        (insert "{{ define \"content\" }}\n")
        (end-of-buffer)
        (insert "{{ end }}")
        (save-buffer)
        (kill-buffer (current-buffer)))))
  :bind (("C-c M-e M-e" . index/ox-html)))
...
See `org-capture-templates' for more information."
  ...
  (add-to-list 'org-capture-templates
               '("b"
                 "Blog post"
                 entry
                 (file+olp "~/web/website.org" "Blog Posts")
